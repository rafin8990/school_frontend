...
const EditnoticeModal = ({ data }: { data: any }) => {
    ...
    const { data: editableData, isLoading }: { data: any, isLoading: any } = useGetById('/notice', 'notice', data?.id);

    const { mutateAsync: updateNotice, isPending: isUpdating } = useUpdate(
        '/notice',
        'notice',
      );
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // const response = await fetch("http://localhost:5501/api/v1/notice/" + data._id, {
            //     method: "PATCH",
            //     body: formData,
            // });
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                <button className="bg-blue-600 text-white hover:bg-gray-300 hover:text-black px-5 py-1 rounded-md flex justify-center items-center gap-2"> <EditIcon /> Edit Notice</button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="text-center text-teal-600 text-2xl">
                        Update Notice
                    </DialogTitle>
                </DialogHeader>
